    ...
    namespace detail
    {
        ...
        struct auto_size_type
        {
            using type = typename std::conditional<
                (Size <= std::numeric_limits<unsigned char>::max()), unsigned char,
        ...
        // For this we could just ignore the first element of the array (still needing a specialization, since that element must not be
        // constructed for non-pod types), however, if the current size fits into this spot, we can avoid wasting memory and still profit
        // from one based arrays in heaps. That's why we have specializations on the involved sizes. The template works on sizes and aligns
        // to reduce type bloat.

        template <size_t SizeOfT, size_t AlignOfT, size_t Size, size_t Base,
                  bool InArraySize = (SizeOfT * Base >= sizeof(auto_size_type_t<Size>) && AlignOfT >= alignof(auto_size_type_t<Size>))>
